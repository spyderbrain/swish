...
env.cmdline.setTheme("ace/theme/textmate");
...
function consult_and_ask() {
    ...
    if (query) {
    	...
    	updateHistory(query);
    	env.prolog.consult_and_ask(program, query)
    }    
}
...
function handlePrompt() {
    ...
    reader.focus();
    $("#prompt").html(this.msg);
    disableButtons(true, true, true, false);
}
function handleOutput() {
	var data = this.msg.trim();
	data = data.replace(/^'/, "");
	data = data.replace(/'$/, "");
...
function handleError() {
    var html = highlight2("% " + this.msg) + "<br />";
    addmsg(html, "solution");
    addmsg("?- ", "goal");
    disableButtons(false, true, true, false);
}
...
$("#font-family-menu").on("change", function() {
	...
	setFontFamily(value);
	if (localStorage) {
		localStorage['font-family'] = value;
	}
});
...
$("#line-highlight-checkbox").on("change", function() {
	...
	if (localStorage) {
		localStorage['line-highlight'] = value;
	}
});
...
$("#line-numbering-checkbox").on("change", function() {
	...
	setShowGutter(value);
	if (localStorage) {
		localStorage['line-numbering'] = value;
	}
});
...
$("#ask-btn").on("click", consult_and_ask);
...
$(document).ready(function() {
	if (localStorage && localStorage.length > 0) {
		...
		setFontFamily(localStorage['font-family']);
		setFontSize(localStorage['font-size']);
		setTabSize(parseInt(localStorage['tab-size'], 10));
		setLineWrap(parseBoolean(localStorage['line-wrap']));
		setLineHighlight(parseBoolean(localStorage['line-highlight']));
		setShowGutter(parseBoolean(localStorage['line-numbering']));
		setUseSoftTabs(parseBoolean(localStorage['tab-soft']));
	}
    maybeLoadSrc();
    addmsg("?- ", "goal");
	setGoal("?- ");
